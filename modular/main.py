    ...
    conv_handler = ConversationHandler(
        ...
        states={
            INIT: [
                ...
                CallbackQueryHandler(start_check_trades, pattern='^check_previous_trades$')
            ],
            ...
            PHOTO: [MessageHandler(filters.TEXT & ~filters.COMMAND, photo_handler)],
            SAVE: [MessageHandler(filters.PHOTO & ~filters.COMMAND, save_trade_handler)],
            CHECK_TRADES: [
                CallbackQueryHandler(check_trade_by_date_range_handler, pattern='^date_range$'),
                CallbackQueryHandler(check_trade_by_id_handler, pattern='^specific_trade$'),
                CallbackQueryHandler(check_trades_by_ticker_handler, pattern='^ticker$'),
                CallbackQueryHandler(check_trades_by_side_handler, pattern='^trade_side$'),
                CallbackQueryHandler(check_trades_by_status_handler, pattern='^trade_status$'),
            ],
        },
